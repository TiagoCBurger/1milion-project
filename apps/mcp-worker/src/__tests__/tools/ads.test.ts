...
describe("Ad Tools", () => {
  describe("get_ads", () => {
    ...
    beforeEach(() => {
      ...
      registerAdTools({ server: capture.server, token: TOKEN, tier: "pro", env: createMockEnv(), organizationId: "test-ws", enableMetaMutations: true });
    });
  ...
  describe("create_ad (tier gating)", () => {
    it("blocks free tier", async () => {
      const capture = createToolCapture();
      registerAdTools({ server: capture.server, token: TOKEN, tier: "free", env: createMockEnv(), organizationId: "test-ws", enableMetaMutations: true });
