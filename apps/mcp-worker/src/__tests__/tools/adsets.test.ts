...
describe("Adset Tools", () => {
  describe("get_adsets", () => {
    ...
    beforeEach(() => {
      ...
      registerAdsetTools({ server: capture.server, token: TOKEN, tier: "pro", env: createMockEnv(), workspaceId: "test-ws" });
    });
  ...
  describe("create_adset (tier gating)", () => {
    it("blocks free tier", async () => {
      const capture = createToolCapture();
      registerAdsetTools({ server: capture.server, token: TOKEN, tier: "free", env: createMockEnv(), workspaceId: "test-ws" });
